...
struct FireBulletDatazxcczx
{
	FireBulletDatazxcczx(const Vector& eyePos, C_BaseEntity* entity) : src(eyePos)
	{
	}
...
void ScaleDamage(int hitgroup, C_BasePlayer* enemy, float weapon_armor_ratio, float& current_damage)
{
	...
	if (enemy->m_ArmorValue() > 0.0f && hitgroup < HITGROUP_LEFTLEG)
	{
		if (hitgroup == HITGROUP_HEAD && !enemy->m_bHasHelmet())
			return;

		float armorscaled = (weapon_armor_ratio * 0.5f) * current_damage;
		if ((current_damage - armorscaled) * 0.5f > enemy->m_ArmorValue())
			armorscaled = current_damage - (enemy->m_ArmorValue() * 2.0f);
...
bool TraceToExitalt(Vector& end, trace_t& tr, Vector start, Vector vEnd, trace_t* trace)
{
...
bool HandleBulletPenetratio22n(CCSWeaponInfo* wpn_data, FireBulletDatazxcczx& data, bool extracheck, Vector point)
{
...
bool SimulateFireBullet(C_BaseEntity* local, C_BaseCombatWeapon* weapon, FireBulletDatazxcczx& data)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		if (data.enter_trace.fraction == 1.0f)
			break;
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0) && (local->m_iTeamNum() != data.enter_trace.hit_entity2->m_iTeamNum()))
		{
			data.trace_length += data.enter_trace.fraction * data.trace_length_remaining;
			data.current_damage *= pow(wpn_data->flRangeModifier, data.trace_length * 0.002);
			ScaleDamage(data.enter_trace.hitgroup, data.enter_trace.hit_entity2, wpn_data->flArmorRatio, data.current_damage);
			return true;
		}
		if (!HandleBulletPenetratio22n(wpn_data, data, false, Vector(0, 0, 0)))
			break;
	}
	return false;
}
...
bool CanHitFloatingPoint(const Vector& point, const Vector& source) // ez
{
	if (!g_LocalPlayer)
		return false;

	FireBulletDatazxcczx data = FireBulletDatazxcczx(source, g_LocalPlayer);
	...
	if (VectortoVectorVisible(data.src, point) || HandleBulletPenetratio22n(weaponData, data, true, point))
		return true;
