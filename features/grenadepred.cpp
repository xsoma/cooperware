...
{
	if (!Variables.Visuals.greane_prediction)
		return;
...
void CCSGrenadeHint::Paint()
{
	...
	if ((type) && path.size() > 1 && act != ACT_NONE && weapon->IsGrenade())
	{
		...
		{
			Visuals::Get().Draw3dCube(2.f, it->second, it->first, Color(0, 255, 0, 200));
		}

		Visuals::Get().Draw3dCube(2.f, OtherCollisions.rbegin()->second, OtherCollisions.rbegin()->first, Color(0, 0, 137, 200));
		...
		if (bestdmg > 0.f) {
			if (weap_id != WEAPON_HEGRENADE)
			{
				if (Math::WorldToScreen(prev, cd))
					Render::Get().DrawString(Render::Get().esp_font, cd[0], cd[1] - 10, Color(0,255,137), FONT_CENTER, firegrenade_didnt_hit ? "no target" : (EntName + " will burn").c_str());
			}
			else
			{
				if (Math::WorldToScreen(*path.begin(), cd))
					Render::Get().DrawString(Render::Get().esp_font, cd[0], cd[1] - 10, Color(0, 255, 137), FONT_CENTER, ("love and affection to " + EntName + " -" + std::to_string(bestdmg)).c_str());
			}
		}
	}
}

void CCSGrenadeHint::Setup(C_BasePlayer* pl, Vector& vecSrc, Vector& vecThrow, const Vector& angEyeAngles)
{
...
void CCSGrenadeHint::Simulate(Vector& Angles, C_BasePlayer* pLocal)
{
	...
	TracerColor = Color(0, 255, 255, 255);
...
int CCSGrenadeHint::Step(Vector& vecSrc, Vector& vecThrow, int tick, float interval)
{
	...
	{
		TracerColor = Color(255, 0, 255, 255);
	}
